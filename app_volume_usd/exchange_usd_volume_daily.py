...
def usd_volume_report():
    ...
    for t in ticker_cb:
        ...
        try:
            data_prev = host_2.query_tables(measurement, ["*","where exchange = 'coinbase' and symbol = '{}' and time >= now() - 1d order by time desc limit 1".format(t)],"raw")[0]['volume']
        except IndexError:
    ...
    for t in ticker_kr:
        ...
        try:
            data_prev = host_2.query_tables(measurement, ["*","where exchange = 'kraken' and symbol = '{}' and time >= now() - 1d order by time desc limit 1".format(t)],"raw")[0]['volume']
        except IndexError:
            data_new = {t:float(data['volume'])*float(data['last'])}
    ...
    vol_total_prev = host_2.query_tables(measurement, ["*","where exchange = 'agg' and symbol = 'vol_total' and time >= now() - 1d order by time desc limit 1".format(t)],"raw")[0]['volume']
    vol_cb_prev = host_2.query_tables(measurement, ["*","where exchange = 'agg' and symbol = 'vol_cb' and time >= now() - 1d order by time desc limit 1".format(t)],"raw")[0]['volume']
    vol_kr_prev = host_2.query_tables(measurement, ["*","where exchange = 'agg' and symbol = 'vol_kr' and time >= now() - 1d order by time desc limit 1".format(t)],"raw")[0]['volume']
    ...
    df_coinbase = pd.DataFrame([data_cb,data_delta_cb,data_delta_percentage_cb],index=['volume','volume_change_daily','volume_change_percentage']).T
    df_kraken = pd.DataFrame([data_kr,data_delta_kr,data_delta_percentage_kr],index=['volume','volume_change_daily','volume_change_percentage']).T
